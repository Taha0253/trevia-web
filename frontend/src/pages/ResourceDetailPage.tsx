    ...
    <div className="pb-24">
      ...
      <section className="border-b border-[#0E2C52]/80 bg-[#030A14] py-10 sm:py-14">
        <div className="max-w-5xl mx-auto px-6 space-y-4">
          <Link to="/resources" className="inline-flex items-center gap-1.5 text-xs font-mono text-slate-400 hover:text-white transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          <article className="lg:col-span-3 space-y-5 prose-invert">
            {resource.body.map((block, i) => {
              if (block.type === 'h2') {
                ...
                return (
                  <h2 key={i} id={anchor} className="text-lg sm:text-xl font-bold text-white pt-4 scroll-mt-24">
                    {block.text}
                  </h2>
                );
              }
              ...
              if (block.type === 'list') {
                return (
                  <ul key={i} className="space-y-2 text-sm text-slate-300 leading-relaxed list-disc pl-5">
              ...
              if (block.type === 'code') {
                return (
                  <pre key={i} className="bg-[#030A14] border border-[#0E2C52] rounded-xl p-4 overflow-x-auto text-xs font-mono text-[#00A09A] leading-relaxed">
          ...
          {headings.length > 0 && (
            <aside className="hidden lg:block lg:col-span-1">
              <div className="sticky top-28 space-y-3 p-5 rounded-xl bg-[#030A14] border border-[#0E2C52]">
                <div className="text-[10px] font-mono uppercase tracking-wider text-slate-500">On This Page</div>
                  ...
                  {headings.map((h, i) => {
                    ...
                    return (
                      <a
                        key={i}
                        href={`#${anchor}`}
                        className="block text-xs text-slate-400 hover:text-[#00A09A] transition-colors leading-snug"
                      >
                        {h.text}
                      </a>
                    );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {related.map((r) => (
              <Link
                key={r.slug}
                to={`/resources/${r.slug}`}
                className="p-5 rounded-xl bg-[#030A14] border border-[#0E2C52] hover:border-[#00A09A]/50 transition-all space-y-2 group"
              >
                <span className="text-[10px] font-mono uppercase tracking-wider text-[#00A09A]">{r.badge}</span>
                <h4 className="text-sm font-bold text-white group-hover:text-[#00A09A] transition-colors leading-snug">{r.title}</h4>
                <span className="inline-flex items-center gap-1 text-xs font-mono text-slate-400 group-hover:text-white transition-colors">
                  <span>Read</span>
